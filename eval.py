...
def test(config, test_data_loader, gen, criterionMSE, epoch):
    ...
    for i, batch in enumerate(test_data_loader):
        ...
        att, out = gen(x)
        # and filename[0] in get_image_arr(config.datasets_dir)
        if config.modellist == 'test':
            if epoch % config.snapshot_interval == 0 and epoch > 20:
                if(x.shape[1]==3):
    ...
    # log_test['Lpips'] = avg_lpips  # todo 新加入的损失
