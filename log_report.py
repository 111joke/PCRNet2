...
class LogReport():
    ...
    def save_lossgraph(self):
        ...
        for l in self.log_:
            ...
            # gen_loss.append(l['loss_char/loss'])
            # dis_loss.append(l['loss_edge/loss'])
...
class TestReport():
    ...
    def save_lossgraph(self):
        ...
        for l in self.log_:
            epoch.append(l['epoch'])
            mse.append(l['ssim'])  # todo 原始为mse,已经更改
            psnr.append(l['psnr'])
            # Lpips.append(l['Lpips'])  # todo 新加入

        epoch = np.asarray(epoch)
        mse = np.asarray(mse)
        psnr = np.asarray(psnr)
        # Lpips = np.asarray(Lpips)
        ...
        # plt.plot(epoch, Lpips)
        # plt.xlabel('epoch')
        # plt.ylabel('Lpips')
        # plt.savefig(os.path.join(self.log_dir, 'graph_Lpips.pdf'))
        # plt.close()
